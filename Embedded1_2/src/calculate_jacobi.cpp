...
calculate_jacobi::calculate_jacobi(QObject *parent):
    QObject(parent)
{
}
void calculate_jacobi::InitPara(double phi_input, double theta1L_input, double delta1_input, double L1_input,
                           double theta2L_input, double delta2_input)
{
    ...
    L2=25;
    gripper=5;
    v_lim=10;w_lim=2;delta_t=0.01;
    update_inter_para();
}

Matrix<double,6,1> calculate_jacobi::resolved_rates(Matrix<double,4,4> T)
{
    ...
    x_dot<<v_lim*(temp)/temp.norm(),w_lim*rot_r(R_t*R_c.transpose());
...
Matrix<double,6,6> calculate_jacobi::jacobi()
{
...
Matrix<double,4,4> calculate_jacobi::Transformation_matrix()
{
      Matrix<double,4,4>  T;
...
void calculate_jacobi::update_inter_para()
{
...
Matrix<double, 3, 2> calculate_jacobi::calculate_Jv(double thetaL,double delta,double L)
{
...
Matrix<double, 3, 2> calculate_jacobi::calculate_Jw(double thetaL,double delta)
{
...
Matrix<double,3,3> calculate_jacobi::skew_sym(Vector3d vector)
{
...
Matrix<double, 3, 3> calculate_jacobi::Rotdx(double angle)
{
...
Matrix<double,3,3> calculate_jacobi::Rotdy(double angle)
{
    Matrix<double,3,3> Rotd;
...
Matrix<double,3,3> calculate_jacobi::Rotdz(double angle)
{
    Matrix<double,3,3> Rotd;
...
Vector3d calculate_jacobi::rot_r(Matrix3d R)
{
    Vector3d r;
...
double calculate_jacobi::rot_theta(Matrix3d R)
{
    return acos((R.trace()-1)/2);
}

Matrix<double,6,6> calculate_jacobi::pinv(Matrix<double,6,6> J)
{
        SVD<MatrixXd> svd(J);
        VectorXd singular;
