...
void MotorThread::MotorsCheck()
{
    ...
    for(int i=0;i<DevNum;i++)
    {
        qDebug("nodeid check:%x",NodeID.at(i));
        mCanDev->Epos_set_operation_mode(NodeID.at(i),1); //Profile Position Mode 的编号为 1
        qDebug()<<"nodeid:"<<i+11;
    }
    qDebug()<<"EPOS_Mode:PPM";
    mEPOS_Mode=PPM;

    if(mEPOS_State==SDO){
        for(int i=0;i<DevNum;i++)
        {
            if((mCanDev->Epos_read_NodeID(NodeID.at(i)))!=(NodeID.at(i)-0x600)){
    ...
    if(mEPOS_Mode==PPM){
        for(int i=0;i<DevNum;i++)
        {
            if(i!=4){
                ...
                mCanDev->Epos_set_profile_velocity(NodeID.at(i),3000);
            }
            else
            {
...
{
    mCanDev->Epos_send_noparam(NodeID.at(1),0x2000,0x00);
    usleep(20000);
}
